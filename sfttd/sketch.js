...
function drawKolamAsync() {
  return new Promise(async (resolve) => {
    ...
    if (drawRev){
      for (let i = framework.length - 1; i >= 0; i--) {
        await drawKolamAsyncHelper(i, true);
      }
    }
    
    resolve(); // Resolve once the entire drawing is complete
    await resetAndRedraw(); // Reset and redraw after completion
  });
}
...
async function resetAndRedraw() {
  ...
  background("#E8E0D5");
  spacing = int(random(3, 12))*10;
  spacing = parseInt(spacing);
  if (isNaN(spacing) || spacing <= 0) {
    spacing = 50; // Fallback to a default value if input is invalid
  }

  rows = Math.floor(P5_CANVAS_SIZE_H / spacing);
  columns = Math.floor(P5_CANVAS_SIZE_W / spacing);
